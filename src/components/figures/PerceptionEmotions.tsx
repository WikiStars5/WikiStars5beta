...
const EMOTIONS_CONFIG: { key: EmotionKey; label: string; imageUrl: string; colorClass: string }[] = [
  ...
  { key: 'envidia', label: 'Envidia', imageUrl: `https://firebasestorage.googleapis.com/v0/b/wikistars5-2yctr.firebasestorage.googleapis.app/o/emociones%2Fenvidia.png?alt=media&token=940aa136-2235-48db-84d6-2c461730fde5`, colorClass: 'hover:bg-green-400/20 border-green-500 text-green-600' },
...
export const PerceptionEmotions: React.FC<PerceptionEmotionsProps> = ({ figureId, figureName, initialPerceptionCounts }) => {
  ...
  const handleEmotionClick = async (emotionKeyClicked: EmotionKey) => {
    ...
    try {
        const figureDocRef = doc(db, 'figures', figureId);
        const userVoteDocRef = doc(db, 'userPerceptions', `${currentFirebaseUser.uid}_${figureId}`);
        ...
        await runTransaction(db, async (transaction) => {
            ...
            if (newEmotionToSet) {
                transaction.set(userVoteDocRef, { userId: currentFirebaseUser!.uid, figureId, emotion: newEmotionToSet, addedAt: serverTimestamp() });
        ...
        if (!isAnonymous && newEmotionToSet) {
            const achievementResult = await grantEmocionAlDescubiertoAchievement(currentFirebaseUser.uid);
  ...
  return (
    <Card className="border border-white/20 bg-black">
      <CardHeader>
        ...
        <CardDescription>
          Haz clic en una emoción para votar. Tu voto es anónimo.
        </CardDescription>
      </CardHeader>

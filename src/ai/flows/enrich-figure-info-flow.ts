...
const EnrichFigureInfoInputSchema = z.object({
  ...
  currentAlias: z.string().optional().describe('Any existing alias or other names.'),
  ...
  currentStatusLiveOrDead: z.string().optional().describe('Any existing status (e.g., "Alive", "Deceased", "Unknown").'),
  ...
  currentEyeColor: z.string().optional().describe('Any existing eye color.'),
  currentDistinctiveFeatures: z.string().optional().describe('Any existing distinctive features (e.g., "Tall ponytail", "Scar over left eye").'),
});
...
const EnrichFigureInfoOutputSchema = z.object({
  ...
  alias: z.string().optional().describe("Alias or other names for the figure. Provide only the text, no labels."),
  ...
  statusLiveOrDead: z.string().optional().describe("The status of the figure (e.g., 'Alive', 'Deceased', 'Unknown'). Provide only the text, no labels."),
  ...
  eyeColor: z.string().optional().describe("The eye color of the figure. Provide only the text, no labels."),
  distinctiveFeatures: z.string().optional().describe("Distinctive features of the figure (e.g., 'Tall ponytail', 'Scar over left eye'). Provide only the text, no labels."),
});
...
- Current Alias: {{#if currentAlias}}"{{currentAlias}}"{{else}}None{{/if}}
...
- Current Status (Live/Dead): {{#if currentStatusLiveOrDead}}"{{currentStatusLiveOrDead}}"{{else}}None{{/if}}
...
- Current Eye Color: {{#if currentEyeColor}}"{{currentEyeColor}}"{{else}}None{{/if}}
- Current Distinctive Features: {{#if currentDistinctiveFeatures}}"{{currentDistinctiveFeatures}}"{{else}}None{{/if}}
...
Output should be like:
{
  ...
  "statusLiveOrDead": "Alive",
  "maritalStatus": "Single (though part of Issei's harem)",
  "height": "168cm (5'6\")",
  "weight": "54kg (119 lbs)",
  "hairColor": "Black",
  "eyeColor": "Violet",
  "distinctiveFeatures": "Usually wears an orange ribbon in her long black hair, tied in a ponytail. Known for her sadistic tendencies in battle."
}
  ...
  async (input) => {
    ...
    if (!output) {
      return {
        ...
        alias: input.currentAlias || undefined,
        ...
        statusLiveOrDead: input.currentStatusLiveOrDead || undefined,
        ...
        eyeColor: input.currentEyeColor || undefined,
        distinctiveFeatures: input.currentDistinctiveFeatures || undefined,
      };
    }

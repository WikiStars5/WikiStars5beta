...
export const getAllUsers = onCall(async (request) => {
    ...
    if (request.auth?.uid !== ADMIN_UID) {
        throw new HttpsError('permission-denied', 'You must be an admin to access user data.');
    }
    ...
    try {
        ...
        const users = usersSnapshot.docs
            ...
            // 3. Filter out anonymous users directly on the server
        ...
        throw new HttpsError('internal', 'An error occurred while fetching user data from Firestore.', error.message);
    }
});
